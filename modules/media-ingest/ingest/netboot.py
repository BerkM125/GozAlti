"""Self-contained networking bootstrap (ported from experiments/surukamera).

Hackathon venue networks have blocked outbound UDP/53 before (all DNS servers
time out) while TCP works fine. Rather than requiring system config changes,
every outbound HTTP call in this module goes through an in-process CONNECT
proxy that resolves hostnames itself via DNS-over-TCP to public resolvers.
